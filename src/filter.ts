import type { Job, FilterResult } from "./types";
...
export function exec(job: Job, filters: JobFilter[], cfg: typeof config): FilterResult {
...
export class TitleIncludeFilter implements JobFilter {
  passes(job: Job, cfg: typeof config): FilterResult {
    ...
    const hit = keywords.some((kw) => matches(job.title, kw, cfg.filters.matchMode));
    return hit
      ? { passed: true }
      : { passed: false, reason: `title missing required keywords (${keywords.join(", ")})` };
  }
}
...
export class TitleExcludeFilter implements JobFilter {
  passes(job: Job, cfg: typeof config): FilterResult {
    ...
    if (keywords.length === 0) return { passed: true };
    const hit = keywords.find((kw) => matches(job.title, kw, cfg.filters.matchMode));
    return hit
  ...
  passes(job: Job, cfg: typeof config): FilterResult {
    if (job.publishedAt === null) {
      if (cfg.filters.includeIfDateMissing) {
        console.warn(`[warn] job ${job.id} has no publishedAt — including by default`);
        return { passed: true };
      }
    ...
    return ageDays <= cfg.filters.maxAgeDays
      ? { passed: true }
      : { passed: false, reason: `too old (${Math.floor(ageDays)}d > ${cfg.filters.maxAgeDays}d)` };
  }
}
...
export class LocationFilter implements JobFilter {
  passes(job: Job, cfg: typeof config): FilterResult {
    ...
    return { passed: false, reason: `location "${job.location}" not remote or in allowedLocations` };
  }
}
...
export class EmploymentTypeFilter implements JobFilter {
  passes(job: Job, cfg: typeof config): FilterResult {
    ...
    return allowed.includes(job.employmentType)
      ? { passed: true }
      : { passed: false, reason: `employmentType "${job.employmentType}" not in allowedEmploymentTypes` };
  }
}
