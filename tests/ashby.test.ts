...
describe("AshbySource normalisation", () => {
  it("parses all listed jobs from the ramp fixture", async () => {
    const source = new AshbySource(mockFetch(fixtureResponse), 0);
    const jobs = await source.fetch("ramp");
    expect(jobs.length).toBe(
      fixtureResponse.jobs.filter((j: any) => j.isListed !== false).length
    );
  ...
  it("drops isListed === false jobs", async () => {
    ...
    const source = new AshbySource(mockFetch(payload), 0);
  ...
  it("returns [] on 404", async () => {
    const source = new AshbySource(mockFetch(null, 404), 0);
